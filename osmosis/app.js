...
const lipidCount = 124;
const lipids = [];
const insideWaterSpheres = [];
let lastInsideWaterVisible = -1;
let lastCellShapeKey = "";
let lastCellSize = 0;
let lastStageState = "";
let lastDirectionClassKey = "";
...
function buildParticles() {
  ...
  insideWaterSpheres.length = 0;
  lastInsideWaterVisible = -1;
  ...
  particleField.appendChild(particleFragment);
...
function buildInsideWater() {
  ...
  for (let i = 0; i < 26; i += 1) {
    ...
    insideWaterSpheres.push(water);
    fragment.appendChild(water);
  }
  insideWater.appendChild(fragment);
  renderInsideWater();
}

function renderInsideWater() {
  const visibleCount = Math.round(8 + ((internalWater - 55) / 90) * 17);
  if (visibleCount === lastInsideWaterVisible) {
    return;
  }
  lastInsideWaterVisible = visibleCount;
  insideWaterSpheres.forEach((water, index) => {
...
function renderSoluteDistribution() {
  ...
  const insideCount = Math.round(5 + inside * 0.32);
  const outsideCount = Math.round(5 + outside * 0.38);

  cellSolutes.innerHTML = "";
  outsideSolutes.innerHTML = "";
  const insideFragment = document.createDocumentFragment();
  const outsideFragment = document.createDocumentFragment();
  ...
  for (let i = 0; i < insideCount; i += 1) {
    ...
    insideFragment.appendChild(solute);
  }
  ...
  for (let i = 0; i < outsideCount; i += 1) {
    ...
    outsideFragment.appendChild(solute);
  }
  cellSolutes.appendChild(insideFragment);
  outsideSolutes.appendChild(outsideFragment);
}
...
function buildCellWaterMolecules() {
  ...
  for (let i = 0; i < count; i += 1) {
    ...
    const delay = i * 0.12;
    const offset = i % 3;
    fragment.appendChild(createStageWaterMolecule("water-in", delay, angle, offset));
    fragment.appendChild(createStageWaterMolecule("water-out", delay + 0.18, angle + 0.07, offset));
  }
  cellWaterMolecules.appendChild(fragment);
}
...
function createPhospholipids() {
  ...
  for (let i = 0; i < lipidCount; i += 1) {
    ...
    `;
    fragment.appendChild(lipid);
    lipids.push(lipid);
  }
  phospholipidLayer.appendChild(fragment);
}
...
function updateCellShape(state) {
  ...
  const shapeKey = `${state}-${roundedSize}`;
  if (shapeKey === lastCellShapeKey) {
    return;
  }
  lastCellShapeKey = shapeKey;
  lastCellSize = roundedSize;
  const center = size / 2;
  const polygonPoints = [];
...
function updateDirectionClass() {
  ...
  const isRunningClass = lowerWaterMoleculesActive && (running || Math.abs(direction) > 0.18);
  const isInward = direction > 0.18;
  const isOutward = direction < -0.18;
  const key = `${isRunningClass}-${lowerWaterMoleculesActive}-${isInward}-${isOutward}`;
  if (key === lastDirectionClassKey) {
    return;
  }
  lastDirectionClassKey = key;
  stage.classList.toggle("running", isRunningClass);
  stage.classList.toggle("water-active", lowerWaterMoleculesActive);
  stage.classList.toggle("inward", isInward);
  stage.classList.toggle("outward", isOutward);
}
...
function render() {
  ...
  setElementText(insideValue, `${insideSlider.value}%`);
  setElementText(outsideValue, `${outsideSlider.value}%`);
  setElementText(stateLabel, balanced ? t().solutions.equilibrium.name : type.name);
  setElementText(solutionName, balanced ? t().solutions.equilibrium.name : type.name);
  setElementText(solutionDetail, balanced ? t().solutions.equilibrium.detail : type.detail);
  setElementText(explanation, balanced && running === false ? t().solutions.equilibrium.text : type.text);
  cell.style.setProperty("--cell-scale", scale.toFixed(3));
  renderInsideWater();
  if (cellState !== lastStageState) {
